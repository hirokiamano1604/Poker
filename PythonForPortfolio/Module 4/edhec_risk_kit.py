...
def drawdown(return_series: pd.Series):
    """
    It takes a time-series of asset returns
...
def gett_ffme_returns():
    """
    Load the Fama-French Dataset for the returns of the Top and Bottom Deciles by MarketCap 
    """
    me_m = pd.read_csv('../data/Portfolios_Formed_on_ME_monthly_EW.csv', header = 0, index_col = 0,na_values = -99.99)# type: ignore
...
def get_hfi_returns():
    """
    Load and format the EHEC Hedg Fund Index Returns
    """
...
def get_ind_returns():
    """
    Load and format the Ken Frence 30 Industry Portfolios Value Weighted Monthly Returns
    """
...
def get_total_market_index_returns():
    ...
    total_market_index = drawdown(total_market_returns).Wealth
...
def var_historic(r,level =5):
    ...
    fall nelow thet number, and the (100-level) percent are above.
    """
...
def var_gaussian(r,level = 5,modified = False):
    """
    Returns the Prametric Gaussian VaR of a Series or DataFrame
...
def run_cppi(risky_r, safe_r = None, m=3, start = 1000, floor = 0.8, riskfree_rate = 0.03, drawdown = None):
    """
    Run a backtest of the CPPI strategy, given a set of returns for the rsiky asset
...
def summary_stats(r, riskfree_rate = 0.03):
    """
    Return a DataFra,e that contains aggregated summary stats for the returns in the columns of r
    """
...
def bond_cash_flows(maturity, principal = 100, coupon_rate=0.03, coupons_per_year = 12):
    """
    Returns a series of acash flows generated by a bond
